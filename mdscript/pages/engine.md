...
|------|------|-------------|
| `boost.available` | [`boolean`](./boolean.html) | true iff the booster can be activated right now |
| `boost.active` | [`boolean`](./boolean.html) | true iff the booster is currently active |
| `boost.chargetime` | [`time`](./time.html) | Charge time until the boost starts |
| `boost.rechargetime` | [`time`](./time.html) | Time required to recharge after a boost |
| `boost.maxduration` | [`time`](./time.html) | The maximum boost duration possible |
| `boost.maxspeed` | [`length`](./length.html) | Maximum boost speed with present engine set up and conditions (in m/s) |
| `travel.available` | [`boolean`](./boolean.html) | true iff the engine's travel mode can be activated right now |
| `travel.active` | [`boolean`](./boolean.html) | true iff the engine is currently traveling |
...
| `travel.iscoasting` | [`boolean`](./boolean.html) | true iff the engine is currently coasting after turning off travel mode |
